...
const EcnPartsList: React.FC = () => {
  ...
  const [selectedEcn, setSelectedEcn] = useState<{
    ecn_id: number;
    status: string;
    description: string;
  } | null>(null);
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get(`${BASE_URL}api/ecnParts`, {
        withCredentials: true,
      });
  ...
  return (
    <div className="p-4">
      <Breadcrumb pageName="ECN Parts List" />
      <input
        type="text"
