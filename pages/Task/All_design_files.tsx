...
const AllDesignFiles = () => {
  ...
  const [startDate, setStartDate] = useState(''); // Start date for date filter
  const [endDate, setEndDate] = useState(''); // End date for date filter
  ...
  useEffect(() => {
    const fetchDesignFiles = async () => {
      try {
        const response = await axios.get(`${BASE_URL}api/design-upload-versions`);
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter((file) => {
        ...
        const fileMatch = extractFileName(file.file_path).toLowerCase().includes(searchTerm.toLowerCase());
    ...
    if (startDate && endDate) {
      filtered = filtered.filter((file) => {
        const fileDate = new Date(file.timestamp).getTime();
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(filteredFiles.map((file) => ({
      ...
      'File Name': extractFileName(file.file_path),
      'Version': file.version,
      'Comment': file.comment || 'NULL',
      'Timestamp': formatDateTime(file.timestamp),
    })));
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Design Files');
    XLSX.writeFile(workbook, 'AllDesignFiles.xlsx');
  };
  ...
  return (
    <div className="p-4">
      <Breadcrumb pageName="All Design Files" />
            ...
            {currentFiles.length === 0 ? (
              ...
              currentFiles.map((file, index) => (
                <tr key={file.duv_id}>
                  ...
                  <td className="py-4 px-4">{extractFileName(file.file_path)}</td>
                  <td className="py-4 px-4">{file.version}</td>
                  <td className="py-4 px-4">{file.comment || 'NULL'}</td>
                  <td className="py-4 px-4">{formatDateTime(file.timestamp)}</td>
                  <td className="py-4 px-4">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        onClick={() => handleDownloadPreviousVersion(file.file_path)}
                      >
